...
def warn_none_setting(setting):
    warn(
        f"the setting for {setting} is none, consider changing workplace/{CHALL_SETTING}"
    )


def error_invalid_setting(setting):
    error(
        f"the setting for {setting} is invalid, change workplace/{CHALL_SETTING} in order to proceed"
    )
...
def strict_setting(setting):
    ...
    """
    if get_setting(setting) is None or not os.path.exists(
        get_setting_path_from_root(setting)
    ):
